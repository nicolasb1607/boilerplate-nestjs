...
export class AuthApiService {
  ...
  public async sendCode(email: string): Promise<void> {
    try {
      ...
      const code =
        await this.authService.generateVerificationCode(emailNormalized);
      await this.emailService.sendCodeVerificationEmail({
        sender: SENDER_EMAIL,
        recipient: {
          firstName: user.firstName,
      ...
      throw new ApiException('Failed to send verification code', e);
    }
  }
  ...
  public async googleAuthRedirect(user: UserEntity): Promise<JwtToken> {
    try {
      ...
      throw new ApiException('Failed to login with google', e);
    }
  }
}
